...
export async function loadData() {
  ...
  return raw.filter(d => !isNaN(d.valence) && !isNaN(d.year) && d.year >= 1921 && d.year <= 2020);
}
...
export function aggregateByYear(data, fields) {
  const byYear = d3.rollup(
    ...
    rows => {
      ...
      fields.forEach(f => { obj[f] = d3.mean(rows, r => r[f]); });
      return obj;
    },
    d => d.year
  );
...
export function aggregateByDecade(data, fields) {
  const byDecade = d3.rollup(
    data,
    rows => {
      const obj = { decade: rows[0].decade, decade_label: rows[0].decade_label, count: rows.length };
      fields.forEach(f => { obj[f] = d3.mean(rows, r => r[f]); });
      return obj;
    },
    d => d.decade
